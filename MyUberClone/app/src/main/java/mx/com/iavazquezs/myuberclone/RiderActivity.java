            ...
            query.findInBackground(new FindCallback<ParseObject>() {
                ...
                public void done(List<ParseObject> objects, ParseException e) {
                    if(e == null && objects != null && objects.size() > 0) {
                        ...
                        requestActive = false;
                        requestUberButton.setText("Call An Uber");
                    }
                }
            });
        } else {
                                ...
                                if (ContextCompat.checkSelfPermission(RiderActivity.this, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
                                    ...
                                    if (lastKnownLocation != null) {
                                        ...
                                        if(distanceInMiles < 0.01) {
                                            ...
                                            LatLng driverLocatioLatLng = new LatLng(driverLocation.getLatitude(), driverLocation.getLongitude());
                                            LatLng requestLocationLatLng = new LatLng(userLocation.getLatitude(), userLocation.getLongitude());

                                            ArrayList<Marker> markers = new ArrayList<>();
                                            mMap.clear();
                                            markers.add(mMap.addMarker(new MarkerOptions().position(driverLocatioLatLng).title("Driver location")));
